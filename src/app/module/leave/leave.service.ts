...
const getMyLeaves = async (employeeId: string, queryParams: ILeaveQueryParams): Promise<IQueryResult<LeaveRequest>> => {
    const builder = new QueryBuilder<LeaveRequest>(
        prisma.leaveRequest,
        queryParams,
        {
...
const getAllLeaves = async (queryParams: ILeaveQueryParams): Promise<IQueryResult<LeaveRequest>> => {
    const builder = new QueryBuilder<LeaveRequest>(
        prisma.leaveRequest,
        queryParams,
        {
...
const rejectLeave = async (leaveId: string, rejectorId: string, rejectionReason: string) => {
    const leave = await prisma.leaveRequest.findUnique({ where: { id: leaveId } });
...
const cancelLeave = async (leaveId: string, employeeId?: string) => {
    const leave = await prisma.leaveRequest.findUnique({ where: { id: leaveId } });
