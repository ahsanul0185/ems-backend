...
const cronDailyAttendance = async () => {
    const today = (0, attendance_utils_1.getStartOfDayUTC)();
    ...
    if (absentData.length > 0) {
        await prisma_1.prisma.attendance.createMany({ data: absentData, skipDuplicates: true });
    }
    ...
    if (onLeaveUpdates.length > 0) {
        await Promise.all(onLeaveUpdates.map(({ id, leave_request_id }) => prisma_1.prisma.attendance.update({
            where: { id },
            data: {
    ...
    const missingClockOut = await prisma_1.prisma.attendance.findMany({
        where: {
            date: yesterday,
            clock_in_time: { not: null },
            clock_out_time: null,
    ...
    if (missingClockOut.length > 0) {
        await Promise.all(missingClockOut.map(record => {
            const workMinutes = record.clock_in_time
    ...
    console.log(`[CRON] Daily attendance cron done — ${absentData.length} ABSENT created, ${onLeaveUpdates.length} ON_LEAVE updated, ${missingClockOut.length} auto closed`);
};
/**
 * Parses a "HH:MM" string into { hour, minute }.
 * Falls back to defaults if parsing fails.
 */
const parseCronTime = (timeStr, defaultHour, defaultMinute) => {
...
const initializeCrons = () => {
    const start = parseCronTime(env_1.env.CRON_START_TIME, 0, 0);
    node_cron_1.default.schedule(`${start.minute} ${start.hour} * * *`, cronDailyAttendance);
    console.log(`Attendance cron job initialized: runs daily at ${String(start.hour).padStart(2, "0")}:${String(start.minute).padStart(2, "0")}`);
};
